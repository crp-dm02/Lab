...
Следующим этапом является автоматизация добавления мониторинга новых устройств. Здесь есть 3 подхода 
1. ansible и python скрипты 
2. gitops 
3. custom operator 

пробую вариант с gitops 

во-первых, в чем заключается идея

Допустим появляется новое сетевое устройство, предварительные настройки должны включать следующие пункты 
1. настроенные интерфейсы (ip адреса, no shutdown)
2. настроенная маршрутизация (маршрут до сети кластера)
3. включенный snmp 
Кроме того разумеется должны быть маршруты на промежуточных устройствах и самом кластере (TODO нужно рассмотреть варианты добавления маршрутов автоматом, скорее всего bgp)

 после появления сетевой связности встает следующий вопрос, как автоматизировать настройку нового servicemonitor(это в простом случае когда snmp-expoter pod используется один, судя по документации он выдерживает 1000 устройств) 

 какие рассматривались варианты
 1. werf - инструмент от фланта, подходит для широкого круга задач, в частности для сборки образов, но этой задачи не стояло, поэтому избыточно
 2. argocd - достаточно популярная инструмент и именно на ней был сделан выбор
 3. flux - судя по отзывам не такая популярная

Следующие шаги установить argocd, создать repo в github с необходимым содержимым, создать application и соответственно применить его
Про нюансы ниже

## Argocd install

1. В декхаусе argocd идет встроенный как модуль, но поддерживается только в ee версии, следовательно ставим руками 
В доке все описано, за исключением того что нужно настраивать ingress для доступа, там слишком много вариантов, поэтому yaml прилагается (argocd-ingress.yaml)
Смотрим что все поды поднялись в argocd нэймспейсе и есть доступ к ui по argocd.mtuci-lab.sslip.io (предварительно необходимо добавить в /etc/hosts)
2. Подключаем repo GitHub в ui или cli (так как репозиторий публичный, то процедура состоит в указании ссылки)
3. Формируем непосредственно структуру, кладем в templates yaml
4. Создаем application (app.yaml)
5. Проверяем